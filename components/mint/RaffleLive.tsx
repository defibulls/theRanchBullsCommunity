...
const RaffleLive = ({ minted, maxbulls, rafflePlayers }: Props) => {
  ...
  const fetchPrice = async () => {
    const priceInfo = await contract.methods.mintingPrices(9).call()
    setPrice(priceInfo._cost)
  }
  useEffect(() => {
    if (contract) {
      fetchPrice()
